"""Train the SimVP-gSTA goal-conditioned pixel baseline (offline, mock test).

Protocol: the model sees ONLY the start and goal frames. Its input sequence is
the 64-step linear interpolation between them (a fixed deterministic function
of the two conditioning frames, no extra information), and it is trained to
predict the 64 protocol target frames. Loss: L1 on [0,1] frames.
"""
...
class GoalConditioned(Dataset):
    def __init__(self, split_dir, steps=64, input_mode='interp'):
    ...
    def __getitem__(self, index):
        ...
        start = np.asarray(self.start[index], dtype=np.float32)
        goal = np.asarray(self.goal[index], dtype=np.float32)
        start = np.squeeze(start) / 255.0   # stored with a leading [None] axis
        goal = np.squeeze(goal) / 255.0
        assert start.ndim == 3 and goal.ndim == 3
        if self.input_mode == 'interp':
            frames = (1.0 - weights[:, None, None, None]) * start \
                + weights[:, None, None, None] * goal
        else:  # 'inpaint': endpoints only, middle is a zero hole (no motion prior)
            frames = np.zeros((self.steps, start.shape[0], start.shape[1], 3), dtype=np.float32)
            frames[0] = start
            frames[-1] = goal
...
def main():
    ...
    train_set = GoalConditioned(Path(args.data_root) / 'scale_m_train', input_mode=args.input_mode)
    val_set = GoalConditioned(Path(args.data_root) / 'scale_m_validation', input_mode=args.input_mode)
