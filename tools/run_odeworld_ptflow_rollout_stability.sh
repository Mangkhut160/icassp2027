#!/usr/bin/env bash
# Wrapper for tools/train_odeworld_ptflow_rollout_stability.py.
#
...
OUTPUT_DIR=${1:?Usage: $0 OUTPUT_DIR [trainer args]}
...
exec "$PYTHON" tools/train_odeworld_ptflow_rollout_stability.py "$OUTPUT_DIR" "${@:2}"
